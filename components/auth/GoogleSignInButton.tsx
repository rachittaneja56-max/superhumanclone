...
export function GoogleSignInButton({ callbackUrl = '/inbox' }: { callbackUrl?: string }) {
  ...
  const handleSignIn = async () => {
    ...
    const { error } = await signIn.sso({
      ...
      redirectCallbackUrl: callbackUrl || '/inbox',
    })

    if (error) {
      console.error('[Auth] Clerk SSO start failed', error)
  ...
  return (
    <button
      ...
      className={`
        ...
        hover:opacity-90 hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:scale-100
      `}
    >
      ...
      <div className="relative z-10 flex h-full w-[calc(100%-2px)] items-center justify-center gap-3 rounded-[6px] bg-accent">
        <svg
