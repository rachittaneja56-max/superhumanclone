  ...
  return (
    <>
      <div className="group fixed bottom-4 right-4 z-50 flex flex-col items-end gap-2">
        ...
        <span className="pointer-events-none select-none rounded-full border border-border bg-surface/90 px-3 py-1 text-[11px] font-medium text-foreground-muted opacity-75 shadow-sm backdrop-blur transition-opacity duration-200 group-hover:opacity-100">
          Need help?
        </span>
      </div>

      {agentPanelOpen && !isMailRoute && (
        <aside className="fixed bottom-20 right-4 z-50 flex max-h-[min(78vh,44rem)] w-[min(24rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-[0_24px_80px_rgba(0,0,0,0.45)]">
